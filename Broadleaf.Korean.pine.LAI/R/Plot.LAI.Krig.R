Plot.LAI.Krig=function (minx, maxx, miny, maxy, b, seq, r) 
{
  ...
  LAI.single = function(a, b, r) {
    ...
    LAI = sum(HS_LAI, LS_LAI, YS_LAI, ZD_LAI, KD_LAI, MGL_LAI, 
              ...
              JSQ_LAI, NJQ_LAI, BNQ_LAI, CY_LAI, BH_LAI, HH_LAI, 
              FH_LAI, LYY_LAI, QT_LAI)
    Needles_LAI = sum(HS_LAI, LS_LAI, YS_LAI)
    Broadleaf_LAI = sum(ZD_LAI, KD_LAI, MGL_LAI, SQL_LAI, 
                        HTQ_LAI, HBL_LAI, SMQ_LAI, QKQ_LAI, JSQ_LAI, NJQ_LAI, 
                        BNQ_LAI, CY_LAI, BH_LAI, HH_LAI, FH_LAI, LYY_LAI, 
                        QT_LAI)
    N_L_percent = Needles_LAI/LAI * 100
    B_L_percent = Broadleaf_LAI/LAI * 100
    Species_LAI = c(HS_LAI, LS_LAI, YS_LAI, ZD_LAI, KD_LAI, 
                    MGL_LAI, SQL_LAI, HTQ_LAI, HBL_LAI, SMQ_LAI, QKQ_LAI, 
                    JSQ_LAI, NJQ_LAI, BNQ_LAI, CY_LAI, BH_LAI, HH_LAI, 
                    FH_LAI, LYY_LAI, QT_LAI)
    Species_LAI = matrix(Species_LAI, 1, )
    colnames(Species_LAI) = c("HS_LAI", "LS_LAI", 
                              "YS_LAI", "ZD_LAI", "KD_LAI", "MGL_LAI", 
                              "SQL_LAI", "HTQ_LAI", "HBL_LAI", 
                              "SMQ_LAI", "QKQ_LAI", "JSQ_LAI", 
                              "NJQ_LAI", "BNQ_LAI", "CY_LAI", 
                              "BH_LAI", "HH_LAI", "FH_LAI", "LYY_LAI", 
                              "QT_LAI")
    N_B_LAI = c(Needles_LAI, Broadleaf_LAI, N_L_percent, 
                B_L_percent)
  ...
  p2 = ggplot() + geom_raster(data = Basexyz, aes(x = x, y = y, 
                                                  fill = Vaule)) + theme_bw() + scale_fill_gradientn(colours = c("white", 
                                                                                                                        "green1", "green2", "green3", "green4"))
                                                                                                                        p2 = p2 + labs(title = "LAI") + scale_x_continuous(expand = c(0, 
