...
{
  library(stats)
  mult.point.Voronoi.LAI=function (minx, maxx, miny, maxy, boundary,a, b, r) 
  {
    ...
    background=Voronoi.LAI(minx, maxx, miny, maxy, boundary, b, r) 
    
    data=rbind(background[,1:2],a)
    distp=as.matrix( dist(data,diag = F, upper = T),nrow(data),nrow(data))
    distp[which(distp==0)]=NA
    dbg=distp[1:nrow(background),(nrow(background)+1):nrow(data)]
    
    tree=matrix(NA,nrow(a),6)
    tree=as.data.frame(tree)
    for (i in 1:nrow(a))
    {
      tree[i,]=background[which.min(dbg[,i]) ,]
    }
    tree=tree[,-c(3,5)]
    LAI=cbind(a,tree)
    colnames(LAI)=c("pointx","pointy","treex","treey","Species","LAI")
    LAI
  }
  ...
  Canopy=Local1[which(!Local1$LAI==0),]
  ...
  result=data.frame(a$x,a$y,Local[1,6],min(Local1$LAI), max(Local1$LAI),mean(Local1$LAI),sd(Local1$LAI),nrow(LX)/nrow(Local1),nrow(Canopy)/nrow(Local1),N_L_Percent,B_L_Percent)
  colnames(result)=c("x","y","LAI","Local_min_LAI","Local_max_LAI","Local_mean_LAI","Local_sd_LAI","Gap_percent","Canopy_percent","N_L_Percent","B_L_Percent")
  result
}

result= Local.single.point.Voronoi.LAI (minx, maxx, miny, maxy, boundary,a[1,], b, r, Lr)
pb = tkProgressBar("", "Percent complete %", 0, 100)
for(i in 2:nrow(a))
{
  result=rbind(result,Local.single.point.Voronoi.LAI (minx, maxx, miny, maxy, boundary,a[i,], b, r, Lr))
  info = sprintf("Percent complete %d%%", round(i * 100/nrow(a)))
  setTkProgressBar(pb, i * 100/nrow(a), "LAI", info)
}
close(pb)
result
}
