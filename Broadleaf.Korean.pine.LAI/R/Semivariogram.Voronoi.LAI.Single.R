Semivariogram.Voronoi.LAI.Single=function (minx, maxx, miny, maxy, boundary, b, r, seq) 
{
  ...
  Voronoi_LAI = Voronoi.LAI.mult(minx, maxx, miny, maxy, 0, 
                                 b, r)
  pointx = seq(minx + boundary, maxx - boundary, length.out = seq + 
                 1)
  pointy = seq(miny + boundary, maxy - boundary, length.out = seq + 
                 1)
  ...
  minyb = miny + boundary
  maxyb = maxy - boundary
  xgrid.cen = seq(minxb + 0.5 * (maxxb - minxb) - 0.5/seq * 
                    (maxxb - minxb), minxb + 0.5 * (maxxb - minxb) + 0.5/seq * 
                    (maxxb - minxb), length.out = 3)
  ygrid.cen = seq(minyb + 0.5 * (maxyb - minyb) - 0.5/seq * 
                    (maxyb - minyb), minyb + 0.5 * (maxyb - minyb) + 0.5/seq * 
                    (maxyb - minyb), length.out = 3)
  basexy.cen = expand.grid(xgrid.cen, ygrid.cen)
  xgrid.left.top = seq(minxb + 0.25 * (maxxb - minxb) - 0.5/seq * 
                         (maxxb - minxb), minxb + 0.25 * (maxxb - minxb) + 0.5/seq * 
                         (maxxb - minxb), length.out = 3)
  ygrid.left.top = seq(minyb + 0.75 * (maxyb - minyb) - 0.5/seq * 
                         (maxyb - minyb), minyb + 0.75 * (maxyb - minyb) + 0.5/seq * 
                         (maxyb - minyb), length.out = 3)
  basexy.left.top = expand.grid(xgrid.left.top, ygrid.left.top)
  xgrid.left.bottom = seq(minxb + 0.25 * (maxxb - minxb) - 
                            0.5/seq * (maxxb - minxb), minxb + 0.25 * (maxxb - minxb) + 
                            0.5/seq * (maxxb - minxb), length.out = 3)
  ygrid.left.bottom = seq(minyb + 0.25 * (maxyb - minyb) - 
                            0.5/seq * (maxyb - minyb), minyb + 0.25 * (maxyb - minyb) + 
                            0.5/seq * (maxyb - minyb), length.out = 3)
  basexy.left.bottom = expand.grid(xgrid.left.bottom, ygrid.left.bottom)
  xgrid.right.top = seq(minxb + 0.75 * (maxxb - minxb) - 0.5/seq * 
                          (maxxb - minxb), minxb + 0.75 * (maxxb - minxb) + 0.5/seq * 
                          (maxxb - minxb), length.out = 3)
  ygrid.right.top = seq(minyb + 0.75 * (maxyb - minyb) - 0.5/seq * 
                          (maxyb - minyb), minyb + 0.75 * (maxyb - minyb) + 0.5/seq * 
                          (maxyb - minyb), length.out = 3)
  basexy.right.top = expand.grid(xgrid.right.top, ygrid.right.top)
  xgrid.right.bottom = seq(minxb + 0.75 * (maxxb - minxb) - 
                             0.5/seq * (maxxb - minxb), minxb + 0.75 * (maxxb - minxb) + 
                             0.5/seq * (maxxb - minxb), length.out = 3)
  ygrid.right.bottom = seq(minyb + 0.25 * (maxyb - minyb) - 
                             0.5/seq * (maxyb - minyb), minyb + 0.25 * (maxyb - minyb) + 
                             0.5/seq * (maxyb - minyb), length.out = 3)
  basexy.right.bottom = expand.grid(xgrid.right.bottom, ygrid.right.bottom)
  point = rbind(point, basexy.cen, basexy.left.top, basexy.left.bottom, 
