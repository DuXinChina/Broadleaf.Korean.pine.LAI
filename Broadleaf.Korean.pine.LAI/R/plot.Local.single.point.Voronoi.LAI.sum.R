...
print(Local.single.point.Voronoi.LAI.sum(minx, maxx, miny, maxy, boundary,a,b,strata,r,Lr))
  ...
  library(tcltk)
  library(deldir)
  raster.Voronoi.LAI=function (minx, maxx, miny, maxy, b, r)
  {
  b=b[,-4]
  options(warn=-1)
  Voronoi.LAI.mult = function(minx, maxx, miny, maxy, b, r) 
  {
    dis = r/2 * sqrt(3)
    xpoint = seq(minx-2*r, maxx+2*r, 2 * dis)
    ypoint = seq(miny-2*r, maxy+2*r, 1.5 * r)
...
shape_r1=raster.Voronoi.LAI(minx, maxx, miny, maxy,Forest_strata[[1]], r[1])
  
pb = tkProgressBar("Finsih", "Percent complete %",  0, 100)
for (i in 2:length(Forest_strata))
{
  shape_r=raster.Voronoi.LAI(minx, maxx, miny, maxy,Forest_strata[[i]], r[i])
  shape_r1= overlay(shape_r1, shape_r, fun=function(x,y){return(x+y)})
  info = sprintf("Percent complete %d%%", round(i *  100/length(Forest_strata)))
