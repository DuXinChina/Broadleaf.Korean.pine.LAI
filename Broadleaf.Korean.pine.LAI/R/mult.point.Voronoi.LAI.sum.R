...
mult.point.Voronoi.LAI.sum=function(minx, maxx, miny, maxy, boundary,a,b,strata,r)
{
  library(tcltk)
  library(deldir)
Single.point.Voronoi.LAI.sum=function(minx, maxx, miny, maxy, boundary,a,b,r)
{
  ...
  LAIsum=point[,-c(1,2)]
  LAIsum=LAIsum[,1:length(Forest_strata)*2]
  LAIsum1=sum(LAIsum)
  
  weightLAI=matrix(NA,length(Forest_strata),1)
  for(i in 1:length(Forest_strata))
  {
    weightLAI[i,] =LAIsum[,i]*i
  }
  Strata_cont=sum(weightLAI)/(LAIsum1*(length(Forest_strata)+1)/2)
  point = cbind(point,LAIsum1,Strata_cont)
  
  colnames(point) = c("x", "y", rep(c("Species","LAI"),length(strata)-1),"SumLAI","Strata_cont")
  point
}

result=Single.point.Voronoi.LAI.sum(minx, maxx, miny, maxy, boundary,a[1,1:2],b,r)
pb = tkProgressBar("", "Percent complete %", 0, 100)
for(i in 2:nrow(a))
{
  result=rbind(result,Single.point.Voronoi.LAI.sum(minx, maxx, miny, maxy, boundary,a[i,1:2],b,r))
