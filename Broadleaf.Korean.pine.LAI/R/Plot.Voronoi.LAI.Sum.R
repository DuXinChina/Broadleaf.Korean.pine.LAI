Plot.Voronoi.LAI.Sum=function (minx, maxx, miny, maxy, boundary, b, strata, r) 
{
  ...
  pb = tkProgressBar("Strata1", "Percent complete %", 
                     0, 100)
  for (i in 2:nrow(point)) {
    ...
    info = sprintf("Percent complete %d%%", round(i * 
                                                    100/nrow(point)))
    setTkProgressBar(pb, i * 100/nrow(point), "Strata1", 
                     info)
  }
  Lbnew1 = Lbnew1$LAI
  close(pb)
  for (j in 2:length(Forest_strata)) {
    ...
    pb = tkProgressBar(paste("Strata", j), "Percent complete %", 
                       0, 100)
    for (i in 2:nrow(point)) {
      Lbnew = subset(stra_single, stra_single[, 1] >= (point[i, 
      ...
      info = sprintf("Percent complete %d%%", round(i * 
                                                      100/nrow(point)))
      setTkProgressBar(pb, i * 100/nrow(point), paste("Strata", 
                                                      j), info)
    }
    Lbnewn = Lbnewn$LAI
  ...
  ggplot() + geom_raster(data = point, aes(x = x, y = y, fill = LAI)) + 
    theme_bw() + scale_fill_gradientn(colours = c("white", 
                                                         "yellow2", "green", "green3", "green4")) + 
