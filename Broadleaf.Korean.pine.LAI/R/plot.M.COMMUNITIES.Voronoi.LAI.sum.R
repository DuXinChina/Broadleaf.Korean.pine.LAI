plot.M.COMMUNITIES.Voronoi.LAI.sum=function(minx,maxx,miny,maxy,boundary,a,b,strata,r,Min_com_edge)
{
options(warn=-1)
library(grDevices)

  M.COMMUNITIES.Voronoi.LAI.sum=
    function (minx, maxx, miny, maxy, boundary, a, b, strata, r, Min_com_edge) 
    {
      library(tcltk)
      library(deldir)
      ...
      ConvexHull_point=function(a,Min_com_edge)
      {
        ...
        total_point=rbind(up_point,left_point,right_point,down_point)
        ...
        #计算方形点阵。并根据凸包筛选凸包内的点
        #计算点阵
        point_x=seq(min(hull_vertices[,1]),max(hull_vertices[,1]),length.out=20)
        point_y=seq(min(hull_vertices[,2]),max(hull_vertices[,2]),length.out=20)
...
geom_point(data=pointa,aes(x=x,y=y),size=.8)+
...
geom_polygon(data=hull_vertices,aes(x,y),col="red4",fill="white",alpha=0)
}
